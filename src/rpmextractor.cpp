...
{
public:
    AppDataParser(const char* RpmFilePath, const QString& appdataFilePath);
...
AppDataParser::AppDataParser(const char* RpmFilePath, const QString& appdataFilePath)
{
    ...
    char* buf = nullptr;
    bool ok = rpm_read_file_into_buffer_following_symlinks(RpmFilePath,
...
{
public:
    DesktopFileParser(const char* RpmFilePath, const QString& desktopFilePath);
...
DesktopFileParser::DesktopFileParser(const char* RpmFilePath, const QString& desktopFilePath)
{
...
RPMExtractor::RPMExtractor(QObject* parent)
    : ExtractorPlugin(parent)
{
}

QStringList RPMExtractor::mimetypes() const
{
    return QStringList{
        QStringLiteral("application/x-rpm"),
        QStringLiteral("application/vnd.rpm"),
    };
}

void KFileMetaData::RPMExtractor::extract(ExtractionResult* result)
{
    const auto RpmFilePath = result->inputUrl().toUtf8();
    const auto RpmType = rpm_get_type(RpmFilePath.constData(), false);
    // not a valid rpm file?
    if (RpmType <= 0) {
        return;
    }
    ...
    // and we just can try as the other rpm tools to simply use the first file of the type found
    char** filePaths = rpm_list_files(RpmFilePath.constData());
    if (!filePaths) {
        return;
    }
    ...
    rpm_string_list_free(filePaths);

    // extract data from both files...
    const AppDataParser appData(RpmFilePath.constData(), appdataFilePath);

    const DesktopFileParser desktopFileData(RpmFilePath.constData(), desktopFilePath);
